...
```
What we can see in this output are the keywords, the statistical scores (we have used the default measures - see LADAL's [Keyness and Keyword Analysis in R](https://ladal.edu.au/tutorials/key/key.html)) for more detail), the number of times the keyword appears in the target text, and the number of times the keyword appears in the reference text.  Now that the markup symbols are gone, we are left with the true keywords in each communication mode.
