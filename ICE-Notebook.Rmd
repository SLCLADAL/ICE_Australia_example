...
It's important to note that decisions around what to include and what to remove are part of your analysis. This is not just data cleaning; these choices aren't necessarily clear and they will depend on your research questions and aims. Any changes to your data should be carefully considered not just within the context of the computational techniques you want to apply, but within the context of your research. For a more critical discussion of these issues, see Rawson and Muñoz's 'Against Cleaning' (https://www.jstor.org/stable/pdf/10.5749/j.ctvg251hk.26.pdf?acceptTC=true&coverpage=false). 

For our purposes, we are going to remove any untranscribed text ("<O>...</O>"), extra-corpus text ("<X>...</X>"), formatting text ("<*>...</*>"), and editorial comments ("<&>...</&>"). Then, we will remove all individual markup tags. This allows us to remove the text in between the first lot of tags (as we have deemed this to be not part of the data we are currently investigating), while leaving the text in between other tags (such as overlapping turns). The code below uses regular expressions and the stringr package to remove markup tags - see the LADAL tutorials String Processing in R (https://ladal.edu.au/tutorials/string/string.html) and Regular Expressions in R (https://ladal.edu.au/tutorials/regex/regex.html) for more detail on these functions.
...
This notebook has described just a couple of analytic methods that could be used with the ICE-AUS corpus. Here, we have only looked at the Australian component of the ICE project, comparing across communication modes and subcorpora. However, the common design of ICE corpora also provides an opportunity to compare different varieties of English using different ICE components (e.g. https://www.tandfonline.com/doi/full/10.1080/07268602.2020.1823817?; https://www.tandfonline.com/doi/full/10.1080/13488678.2014.908529?). The ICE project website has information on getting access to various component corpora (https://www.ice-corpora.uzh.ch/en/access.html), as well as a selection of publications (https://www.ice-corpora.uzh.ch/en/publics.html). There are also many more text analytic techniques that could be applied to ICE corpus data; see the Language Technology and Data Analysis Laboratory for tutorials on other text analytic methods (https://ladal.edu.au/tutorials.html).
